...
while 1:
    ...
    if(len(faces) == 0 and len(faces_bw) == 0):
        cv2.putText(img, "No face found...", org, font, font_scale, yes_mask_font_color, thickness, cv2.LINE_AA)
    elif(len(faces) == 0 and len(faces_bw) == 1):
        # It has been observed that for white mask covering mouth, with gray image face prediction is not happening
        cv2.putText(img, yes_mask, org, font, font_scale, yes_mask_font_color, thickness, cv2.LINE_AA)
    else:
        ...
        if(len(mouth_rects) == 0):
            ...
            for (mx, my, mw, mh) in mouth_rects:
                ...
                if(y < my < y + h):
                    ...
                    cv2.putText(img, no_mask, org, font, font_scale, no_mask_color, thickness, cv2.LINE_AA)
